#!/system/bin/sh
# Reactor BootAni (Revision) Boot Animation Service Script
# This runs during boot to ensure proper animation loading

# Log boot animation load
echo "$(date): Reactor BootAni (Revision) boot animation loaded" >> /data/local/tmp/bootfx.log
...
resetprop ro.bootanim.theme "reactor_bootani_revision"
